...
const createEvent = catchAsync(async (req, res) => {
  const event = await eventService.createEvent(req.params.user, req.body);
  res.status(httpStatus.CREATED).send(event);
});

const getEvents = catchAsync(async (req, res) => {
  const paramsFilter = pick(req.params, ['user']);
  const queryFilter = pick(req.query, ['pageId']);
  const filter = { ...paramsFilter, ...queryFilter };
